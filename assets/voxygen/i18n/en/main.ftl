...
main-singleplayer-max_players = Max players (LAN)
main-singleplayer-generation_lane = Generation Mode
main-singleplayer-lane_stable = Stable
main-singleplayer-lane_experimental = ⚠ Experimental
main-singleplayer-experimental_warning = Experimental worlds use the Nova-Forge conversion pipeline. Not guaranteed save-compatible across builds.
main-singleplayer-pvp_mode = Combat Mode
main-singleplayer-pvp = ⚔ PvP
main-singleplayer-pve = 🛡 PvE
main-singleplayer-difficulty = Difficulty
main-singleplayer-difficulty-easy = 🌿 Easy
main-singleplayer-difficulty-normal = ⚔ Normal
main-singleplayer-difficulty-hard = 💀 Hard
...
main-login-username_empty = Username cannot be empty.
main-servers-select_server = Select a server
main-servers-lan_servers = — LAN Servers (auto-discovered) —
main-servers-lan_searching = Scanning for LAN servers…
main-servers-lan_none = No LAN servers found on this network.
...
main-lan_coop-join_hint = Guests can join at: { $address } (no account needed)
main-lan_coop-join_hint_unknown_ip = Guests can join on port { $port } (no account needed; LAN IP could not be detected)
main-lan_coop-play = Host LAN Game
