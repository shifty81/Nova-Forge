            ...
            match &*item.kind() {
                ItemKind::Armor(armor) => {
                            ...
                            ))
                    {
                        ...
                        let energy_reward =
                            if let Some(energy_reward_raw) = record.get(headers["Energy Reward"]) {
                        ...
                        let armor =
                            comp::item::armor::Armor::new(kind, StatsSource::Direct(armor_stats));
                    ...
                    ))
            {
                ...
                let hands = if let Some(hands_raw) = record.get(headers["Hands"]) {
                    match hands_raw {
                        ...
                        "Two" | "2" | "2h" => Hands::Two,
                        _ => {
                            eprintln!("Unknown hand variant for {:?}", item.item_definition_id());
                            Hands::Two
                        },
                    }
                } else {
